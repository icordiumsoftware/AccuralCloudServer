    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h5 class="modal-title" id="myDefaultModalLabel">Select Head</h5>
            </div>
            <div class="modal-body">
                <div class="col-12">
                    <div class="lightboxGallery row text-center" style="padding-left: 5%;" id="heads"></div>
                </div>
            </div>
...
<script type="text/javascript">
    $(function () {
        ...
        $.getJSON('<%=request.getContextPath()%>/areas/all', function (data) {
            var areas = "";
            var areaTab = "";
            $.each(data, function (index, cat) {//areas read...
                areas += "<li class='nav-item' role='presentation'><a class='nav-link " + (index === 0 ? 'active' : '') + "' href='#A" + cat.idareas + "' aria-controls='A" + cat.idareas + "' role='tab' data-toggle='tab' aria-selected='" + (index === 0 ? 'true' : 'false') + "'>" + cat.areasname + "</a></li>";
                areaTab += "<div role='tabpanel' class='tab-pane " + (index === 0 ? 'active' : '') + "' id='A" + cat.idareas + "'><div class='card-body'><div class='lightboxGallery row '>";
                $.each(cat.tablesByIdareas, function (i, typ) {//read tables...
                    areaTab += "<div class='imagebox img-thumbnail img-responsive col-lg-2 col-md-3 col-sm-5' id='T" + typ.idtables + "' onclick='loadHeads(" + typ.idtables + ")'>";
                    areaTab += "<a class='' href='#' data-target='.bs-example-modal-default' data-toggle='modal' ><img  src='assets/img/table.jpg' class='img-fluid' alt=''><span  class='captipnan " + (typ.pending > 0 ? "imagebox-desc-red" : "imagebox-desc") + "'>" + typ.description + "</span></a>";
                    areaTab += "</div>";

                });
                areaTab += "</div></div></div>";
            });
            $("#tabContent").html(areaTab);
            $("#tabs").html(areas);
        });
        //
    });
    function loadHeads(tableId) {
        $.getJSON('<%=request.getContextPath()%>/tableheads/bytaleid/' + tableId, function (data) {
            var heads = "";
            $.each(data, function (index, head) { //read heads...
                heads += "<a href='#' onclick='headSelected(" + tableId + " ,this.id, \"" + head.tableheadsname + "\")'  class='thumbnail " + (head.pending > 0 ? "bg-danger" : "bg-blue") + " col-3 padding-15'  style='margin: 10px;' id='" + head.idtableheads + "'> <span class='h5' >" + head.tableheadsname + "</span></a>";
            });
            $("#heads").html(heads);
        });
    }
    ...
    function headsRed(id) {
        $(id).toggleClass('.bg-blue');
        $(id).toggleClass('.bg-red');
    }
</script>

        ...
        <div class="row">
            <div class="col">
                ...
                <div class="search-form hidden-xs">
                    <form>
                        <input class="form-control" autocomplete="off" placeholder="Search for..." type="text">
...
<div class="row page-header">
    <div class="col-lg-6 align-self-center ">
        <h2>PMenu Sales Dates Table</h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">Home</a></li>
            <li class="breadcrumb-item"><a href="#">Sales</a></li>
            <li class="breadcrumb-item active">Menu Sales Table</li>
        </ol>
    </div>
</div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header card-default">
                    ...
                    <p class="text-muted">Menu Sales Summery List</p>
                    ...
                    <div class="col-md-5 float-right">
                        <div class="input-group m-b">
                            ...
                            <input type="text" id="rentalBookingDateRange"
                                   name="rentalBookingDateRange" value="${startdate}-${enddate}"/>
                            <input type="hidden" id="ragestartdate" value="${startdate}"/>
                            <input type="hidden" id="rageenddate" value="${enddate}"/>
                        </div>
                    </div>
                </div>
...
<script>
    function loardDatatable() {
        $("#datatable").DataTable({
            ...
            "pageLength": 10,
            "ajax": {
                'url': '<%=request.getContextPath()%>/menusalesinvoicedata/location/${sessionScope.get("LocationID")}/ByDate/' + $("#ragestartdate").val() + '/' + $("#rageenddate").val() + '',
                'type': 'GET',
                'data': '',
                'dataSrc': ""
            },
            "footerCallback": function (row, data, start, end, display) {
                ...
                var intVal = function (i) {
                    return typeof i === 'string' ?
                        i.replace(/[\${sessionScope.get("currency")},]/g, '') * 1 :
                        typeof i === 'number' ?
                            i : 0;
                };

                // Total over all pages
                Salestotal = api.column(5).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                discounttotal = api.column(4).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                Costtotal = api.column(2).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                Grosstotal = api.column(3).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);

                // Total over this page
                SalespageTotal = api.column(5, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                discountpageTotal = api.column(4, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                CostpageTotal = api.column(2, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                GrosspageTotal = api.column(3, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);

                // Update footer
                $(api.column(5).footer()).html(
                    ' ${sessionScope.get("currency")} ' + SalespageTotal + ' (  ${sessionScope.get("currency")} ' + SalespageTotal + ' Total)'
                );
                $(api.column(4).footer()).html(
                    ' ${sessionScope.get("currency")} ' + discountpageTotal + ' ( ${sessionScope.get("currency")} ' + discountpageTotal + ' Total)'
                );
                $(api.column(2).footer()).html(
                    ' ${sessionScope.get("currency")} ' + CostpageTotal + ' (  ${sessionScope.get("currency")} ' + CostpageTotal + ' Total)'
                );
                $(api.column(3).footer()).html(
                    ' ${sessionScope.get("currency")} ' + GrosspageTotal + ' (  ${sessionScope.get("currency")} ' + GrosspageTotal + ' Total)'
                );
            },
            "columns": [
                {"data": 'idmenusalesinvoice'},
                {
                    sortable: true,
                    "render": function (data, type, full, meta) {

                        return moment(full.datetime).format("YYYY-MM-DD");
                    }
                },
                {"data": 'costamount'},
                {"data": 'grossamount'},
                ...
                {"data": 'salesamount'},
                {
                    sortable: true,
                    "render": function (data, type, full, meta) {
                        return full.usersIdusers;
                    }
                },
                {
                    sortable: true,
                    "render": function (data, type, full, meta) {
                        return full.tableheadsIdtableheads;
                    }
                }
            ]
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('input[name="rentalBookingDateRange"]').daterangepicker({
            ...
            $("#ragestartdate").val(moment(start).format("YYYY-MM-DD"));
            $("#rageenddate").val(moment(end).format("YYYY-MM-DD"));
            loardDatatable();
        });
